...
try:
    ...
    #with open(file_path, mode='w', newline='') as file:
        ...
        #writer.writerow(["Timestamp","Latitude", "Longitude","Distance1", "Distance2", "Distance3", "Temperature","Humidity","Smoke Concentration", "LPG Concentration", "CO Concentration"])
        # Combine the lists into a single list of rows
        #rows = zip(timestamps, gps_latitudes, gps_longitudes, distances1, distances2, distances3, temperatures, humidities, smoke_values, lpg_values, co_values)  # Add other lists here 
    ...
    #with open('data.csv', 'r') as file2:
        ...
        #for row in reader:
            #print(row)
            #timestamp_file = row[0]
            #latitude_file = float(row[1])
            #longitude_file = float(row[2])
            #dist1_file = float(row[3])
            #dist2_file = float(row[4])
            #dist3_file = float(row[5])
            #temperature_file = float(row[6])
            #humidity_file = float(row[7])
            #smoke_file = float(row[8])
            #lpg_file = float(row[9])
            #co_file = float(row[10])
            #timestamp_list.append(timestamp_file)
            #latitude_list.append(latitude_file)
            #longitude_list.append(longitude_file)
            #dist1_list.append(dist1_file)
            #dist2_list.append(dist2_file)
            #dist3_list.append(dist3_file)
            #temperature_list.append(temperature_file)
            #humidity_list.append(humidity_file)
            #smoke_list.append(smoke_file)
            #lpg_list.append(lpg_file)
            #co_list.append(co_file)
            
    # Create a map object
    #map_center = [latitude_list[0], longitude_list[0]]
    #distance_map = folium.Map(location=map_center, zoom_start=12)


    # Iterate over the data lists
    #for latitude, longitude, dist1, dist2, dist3 in zip(latitude_list, longitude_list, dist1_list, dist2_list, dist3_list):
        # Create a marker
        #marker_distance = folium.Marker(location=[latitude, longitude])

        # Check if any of the distances is smaller than 40cm
        #if dist1 < 40  or dist2 < 40 or dist3 < 40:
            # Set the marker color to red
            #marker_distance = folium.Marker(location=[latitude, longitude], icon=folium.Icon(icon='info-sign', prefix='glyphicon', color='red'))
        #else:
            #marker_distance = folium.Marker(location=[latitude, longitude])

        #popup_html = f"Distance 1: {dist1} cm<br>Distance 2: {dist2} cm<br>Distance 3: {dist3} cm"
        #popup = folium.Popup(html=popup_html, max_width=300)

        # Add the popup to the marker
        #marker_distance.add_child(popup)

        # Add the marker to the map
        #marker_distance.add_to(distance_map)
            # Create a marker
            #marker_distance = folium.Marker(location=[latitude_file, longitude_file])

            # Check if any of the distances is smaller than 40cm
            #if dist1_file < 0.4 or dist2_file < 0.4 or dist3_file < 0.4:
                # Set the marker icon to warning.png
                #icon = folium.Icon(icon='warning', prefix='fa', color='red')
                #marker_distance.icon = icon

            # Create a popup with the distances
            #popup_html = f"Distance 1: {dist1_file} m<br>Distance 2: {dist2_file} m<br>Distance 3: {dist3_file} m"
            #popup = folium.Popup(html=popup_html, max_width=300)
    ...
    # Save the map as an HTML file
       ...
       # Save latitude and longitude lists to a text file
       #with open('coordinates.txt', 'w') as gps_file:
           #for lat, lon in zip(gps_latitudes, gps_longitudes):
               #gps_file.write(f"{lat},{lon}\n")

       #with open('temperature.txt', 'w') as temperature_file:
           #for temp in temperature:
               #temperature_file.write((f'{temp}\n')
 
       # Write humidity data to file
       #with open('humidity.txt', 'w') as humidity_file:
            #for humid in humidity:
                #humidity_file.write(f'{humid}\n')
    ...
    #for lat, lon, temp, humi, smoke, lpg, co in zip(latitude_list, longitude_list, temperature_list, humidity_list, smoke_list, lpg_list, co_list ):
        #popup_content = f'Temperature: {temp}°C<br>Humidity: {humi}%<br>Smoke: {smoke}<br>LPG: {lpg}<br>CO: {co}'
    ...
    #line = folium.PolyLine(locations=marker_locations, color='blue')
    #line.add_to(map)

    #heatmap_data = list(zip(latitude_list, longitude_list, temperature_list))

    #heatmap = folium.plugins.HeatMap(heatmap_data, min_opacity=0.2, max_val=max(temperature_list),
    ...
    #New maps
    #map_center = [latitude_list[0], longitude_list[0]]
    #general_map = folium.Map(location=map_center, zoom_start=12)
 
    #heat map
    #heat_map = folium.Map(location=[latitude_list[0], longitude_list[0]], zoom_start=12)
    #heat_data = list(zip(latitude_list, longitude_list, temperature_list))
    #HeatMap(heat_data).add_to(heat_map)

    

    #Map with all data
    #marker_map = folium.Map(location=[latitude_list[0], longitude_list[0]], zoom_start=12)
    #for lat, lon, temp, humi, smoke, lpg, co in zip(latitude_list, longitude_list, temperature_list, humidity_list, smoke_list, lpg_list, co_list ):
        #popup_content = f'Temperature: {temp}°C<br>Humidity: {humi}%<br>Smoke: {smoke}<br>LPG: {lpg}<br>CO: {co}'
        #marker = folium.Marker(location=[lat, lon], popup=popup_content)
        #marker.add_to(marker_map)
        #marker_locations.append([lat, lon])

    # Map with a line representing the path
    #path_map = folium.Map(location=[latitude_list[0], longitude_list[0]], zoom_start=12)
    #folium.PolyLine(locations=list(zip(latitude_list, longitude_list)), color='blue').add_to(path_map)
    
    #distance_map = folium.Map(location=[latitude_list[0], longitude_list[0]], zoom_start=12)
     
    #for lat, lon, dist1, dist2, dist3 in zip(latitude_list, longitude_list, dist1_list, dist2_list, dist3_list):
    # Create a marker
        #marker_distance = folium.Marker(location=[lat, lon])

    # Check if any of the distances is smaller than 40cm
    #if dist1 < 0.4 or dist2 < 0.4 or dist3 < 0.4:
        # Set the marker icon to warning.png
        #icon = folium.Icon(icon='warning', prefix='fa', color='red')
        #marker_distance.icon = icon

    # Create a popup with the distances
    #popup_html = f"Distance 1: {dist1} m<br>Distance 2: {dist2} m<br>Distance 3: {dist3} m"
    #popup = folium.Popup(html=popup_html, max_width=300)

    # Add the popup to the marker
    #marker_distance.popup = popup

    # Add the marker to the map
    #marker_distance.add_to(distance_map)

    # Save the maps as HTML files
    #general_map.save("base_map.html")
